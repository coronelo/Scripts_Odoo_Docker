...
#   - NUEVO: Sistema de instalación de dependencias desde HOST hacia contenedores
#   - MEJORA CRÍTICA: Sistema de dependencias persistentes en volumen compartido
# =============================================================================
set -euo pipefail
...
# ====== Función corregida para mostrar información del sistema ======
print_system_info() {
    echo -e "${C_CYN}┌──────────────────────────────────────────────────────────┐${C_RESET}"
...
show_credentials() {
    ...
    if [[ -n "$domain" ]]; then
        ...
        for ver in "${ODOO_VERSIONS[@]}"; do
            ...
            print_info_box "🔵 ACCESO A ODOO v${ver}" "URL: http://${LOCAL_IP}:${p_odoo}\n\n🔑 Master Password: ${ADMIN_PASS}\n\n💡 Al acceder por primera vez:\n1. Usa el Master Password arriba\n2. Crea nueva base de datos\n3. Nombre: ${project_name}\n4. Usuario: ${ODOO_USER}\n5. Contraseña: ${ODOO_PASS}"
        done
    fi
...
setup_custom_paths() {
  ...
  # CORRECCIÓN: Inicializar contraseñas ANTES de guardar configuración
  init_global_passwords
...
list_npm_projects() {
    ...
    say "📁 Proyectos en directorio base (${BASE_DIR}):" "$C_MAG"
    if [[ -d "$BASE_DIR" ]]; then
        local count=0
        for dir in "$BASE_DIR"/*/; do
            if [[ -d "$dir" && -f "$dir/docker-compose.yml" ]]; then
...
# Remover proyecto de NPM (CORREGIDA - no falla si NPM no está instalado)
remove_from_npm() {
    local project_name="$1"
...
# ====== NUEVO: SISTEMA DE DEPENDENCIAS PERSISTENTES ======

# Función para instalar dependencias en el volumen persistente
install_dependencies_persistent() {
    local project_name="$1"
    local odoo_version="${2:-19}"
    
    print_section "📦 INSTALACIÓN DE DEPENDENCIAS PERSISTENTES"
    
    # Verificar que el proyecto exista
    local project_dir="${BASE_DIR}/${project_name}"
    if [[ ! -d "$project_dir" ]]; then
        warn "❌ El proyecto '$project_name' no existe en $BASE_DIR"
        return 1
    fi
    
    local python_packages_dir="${project_dir}/python-packages_v${odoo_version}"
    
    # Crear directorio si no existe
    mkdir -p "$python_packages_dir"
    chown -R "$FIXED_UID:$FIXED_GID" "$python_packages_dir"
    
    # Lista de dependencias comunes para Odoo
    local -a common_deps=(
        "google-auth"
    ...
    # Dependencias para módulos de IA
    local -a ai_deps=(
        "torch"
        "transformers"
    ...
    echo -e "${C_CYN}Selecciona las dependencias a instalar en el volumen persistente:${C_RESET}"
    echo -e "  ${C_GRN}1)${C_RESET} ${C_BLU}Google Auth específicamente${C_RESET}"
    echo -e "  ${C_GRN}2)${C_RESET} ${C_BLU}Todas las dependencias comunes (${#common_deps[@]} paquetes)${C_RESET}"
    echo -e "  ${C_GRN}3)${C_RESET} ${C_BLU}Dependencias específicas v${odoo_version} (${#version_specific_deps[@]} paquetes)${C_RESET}"
    echo -e "  ${C_GRN}4)${C_RESET} ${C_BLU}Dependencias de IA (${#ai_deps[@]} paquetes)${C_RESET}"
    echo -e "  ${C_GRN}5)${C_RESET} ${C_BLU}Instalar desde requirements.txt${C_RESET}"
    echo -e "  ${C_GRN}6)${C_RESET} ${C_BLU}Ver dependencias instaladas${C_RESET}"
    echo -e "  ${C_GRN}7)${C_RESET} ${C_BLU}Instalar paquete específico${C_RESET}"
    
    read -p "Opción [1-7]: " dep_option
    
    case "$dep_option" in
        1)
            # Instalar Google Auth
            say "🔐 Instalando Google Auth en volumen persistente..." "$C_MAG"
            pip3 install --target="$python_packages_dir" \
                google-auth google-auth-oauthlib google-auth-httplib2 \
                oauthlib requests-oauthlib
            
            if [[ $? -eq 0 ]]; then
                ok "✅ Google Auth instalado en $python_packages_dir"
                say "💡 Las dependencias estarán disponibles después de reiniciar Odoo" "$C_BLU"
            else
                warn "❌ Error instalando Google Auth"
            fi
            ;;
        2)
            # Instalar todas las dependencias comunes
            say "📦 Instalando ${#common_deps[@]} dependencias comunes..." "$C_MAG"
            for dep in "${common_deps[@]}"; do
                echo -n "  📦 $dep... "
                if pip3 install --target="$python_packages_dir" "$dep" >/dev/null 2>&1; then
                    echo -e "${C_GRN}✅${C_RESET}"
                else
                    echo -e "${C_RED}❌${C_RESET}"
                fi
            done
            ok "✅ Dependencias instaladas en $python_packages_dir"
            ;;
        3)
            # Instalar dependencias específicas de versión
            say "🔧 Instalando dependencias específicas v${odoo_version}..." "$C_MAG"
            for dep in "${version_specific_deps[@]}"; do
                echo -n "  📦 $dep... "
                if pip3 install --target="$python_packages_dir" "$dep" >/dev/null 2>&1; then
                    echo -e "${C_GRN}✅${C_RESET}"
                else
            ...
            ok "✅ Dependencias específicas instaladas"
            ;;
        4)
            # Instalar dependencias de IA
            say "🧠 Instalando dependencias de IA..." "$C_MAG"
            ...
            ok "✅ Dependencias de IA instaladas"
            ;;
        5)
            # Instalar desde requirements.txt
            if [[ -f "${project_dir}/requirements.txt" ]]; then
                say "📄 Instalando desde requirements.txt..." "$C_MAG"
                pip3 install --target="$python_packages_dir" -r "${project_dir}/requirements.txt"
                ok "✅ Dependencias instaladas desde requirements.txt"
            else
                warn "❌ No se encontró requirements.txt en $project_dir"
                say "💡 Creando archivo de ejemplo..." "$C_BLU"
                cat > "${project_dir}/requirements.txt" << EOF
# Dependencias para Odoo ${odoo_version}
google-auth==2.16.0
...
EOF
                ok "✅ requirements.txt creado. Edítalo y vuelve a ejecutar esta opción."
            fi
            ;;
        6)
            # Ver dependencias instaladas
            say "📦 Dependencias instaladas en $python_packages_dir:" "$C_CYN"
            if [[ -d "$python_packages_dir" ]]; then
                ls -la "$python_packages_dir" | grep -E "\.egg-info$|\.dist-info$" | head -20
                echo ""
                say "📊 Total de paquetes:" "$C_BLU"
                find "$python_packages_dir" -name "*.egg-info" -o -name "*.dist-info" | wc -l
            else
                warn "❌ El directorio de dependencias no existe"
            fi
            ;;
        7)
            # Instalar paquete específico
            read -p "Nombre del paquete a instalar: " custom_package
            if [[ -n "$custom_package" ]]; then
                say "📦 Instalando $custom_package..." "$C_MAG"
                pip3 install --target="$python_packages_dir" "$custom_package"
                if [[ $? -eq 0 ]]; then
                    ok "✅ $custom_package instalado"
                else
                    warn "❌ Error instalando $custom_package"
                fi
            fi
            ;;
        *)
    ...
    if [[ -d "$python_packages_dir" ]]; then
        local package_count=$(find "$python_packages_dir" -name "*.egg-info" -o -name "*.dist-info" 2>/dev/null | wc -l)
        if [[ $package_count -gt 0 ]]; then
            say "📊 Resumen:" "$C_GRN"
            say "  📁 Directorio: $python_packages_dir" "$C_BLU"
            say "  📦 Paquetes instalados: $package_count" "$C_BLU"
            say "  🔧 Permisos: $(stat -c "%U:%G" "$python_packages_dir")" "$C_BLU"
            
            # Preguntar si reiniciar Odoo
            echo ""
            read -p "¿Reiniciar contenedor Odoo para aplicar cambios? (s/n): " restart_choice
            if [[ "$restart_choice" =~ ^[sS]$ ]]; then
                local container_name="${project_name}_odoo${odoo_version}"
                if docker ps --format '{{.Names}}' | grep -q "^${container_name}$"; then
                    say "🔄 Reiniciando $container_name..." "$C_YLW"
                    docker restart "$container_name"
                    sleep 5
                    ok "✅ Contenedor reiniciado"
                else
                    warn "⚠️  El contenedor $container_name no está en ejecución"
                fi
            fi
        fi
    fi
...
# ====== MEJORA: Gestión de Versiones de PostgreSQL ======
...
# ====== MEJORA: Cálculo Automático de Recursos ======
...
# ====== MEJORA: Sistema de Logs Centralizado ======
...
# ====== MEJORA: Sistema de Actualización Automática ======
add_update_system() {
    local project_dir="$1"
...
# ====== MEJORA: Sistema de Plugins/Extensiones ======
setup_plugin_system() {
    local project_dir="$1"
...
# ====== MEJORA: Verificaciones para Producción ======
...
# ====== Funciones originales del script (actualizadas) ======

setup_fixed_uids() {
  say "🔧 Configurando UID/GID fijos $FIXED_UID:$FIXED_GID..." "$C_CYN"
  
  # CORRECCIÓN: Solo crear si no existen
  ...
  # CORRECCIÓN: Exportar variables globalmente
...
fix_permissions_with_fixed_uids() {
  ...
  # CORRECCIÓN: Obtener variables del entorno del proyecto
  ...
  # CORRECCIÓN: Definir ODOO_VERSIONS si no existe
  local odoo_versions=("${ODOO_VERSIONS[@]}")
  if [[ ${#odoo_versions[@]} -eq 0 ]]; then
    odoo_versions=("19")  # Valor por defecto
  fi
  ...
  for ver in "${odoo_versions[@]}"; do
    ...
    # CORRECCIÓN: Usar variables consistentes
    ...
    # CORRECCIÓN: Verificar INCLUDE_ENT
    ...
    # NUEVO: Directorio de dependencias Python
    [[ -d "${project_dir}/python-packages_v${ver}" ]] && {
      chown -R "${odoo_uid}:${odoo_gid}" "${project_dir}/python-packages_v${ver}" 2>/dev/null || true
      chmod -R 755 "${project_dir}/python-packages_v${ver}" 2>/dev/null || true
    }
  done
...
prompt_config() {
  ...
  # Usar contraseñas globales
  ODOO_USER="desarrollo"
  ODOO_PASS="${GLOBAL_ODOO_PASS}"
  PG_USER="desarrollo"  
  PG_PASS="${GLOBAL_PG_PASS}"
...
# ====== NUEVA: Función create_structure modificada para incluir directorio de dependencias ======
create_structure() {
  ...
  for ver in "${ODOO_VERSIONS[@]}"; do
    ...
    if [[ "${PERF_MODE}" == "high" ]]; then
      ...
      # CORREGIDO: Configuración PostgreSQL válida
...
EOF
    ...
    # NUEVO: Directorio para dependencias Python persistentes
    mkdir -p "${PROJECT_DIR}/python-packages_v${ver}"
    chown -R "$FIXED_UID:$FIXED_GID" "${PROJECT_DIR}/python-packages_v${ver}"
    ok "    Directorio de dependencias Python v${ver} creado"
    
    # MEJORA: Usar ruta personalizada si está configurada, o la ruta por defecto
    ...
    if [[ "${INCLUDE_ENT}" == "yes" ]]; then
      ...
      # MEJORA: Usar ruta personalizada si está configuradas, o la ruta por defecto
...
# ====== NUEVA: Función write_compose modificada para incluir volumen de dependencias ======
write_compose() {
...
EOF
  ...
  ${PROJECT_NAME}_odoo${ver}:
    ...
    volumes:
      ...
      # NUEVO: Volumen para dependencias Python persistentes
      - ${PROJECT_DIR}/python-packages_v${ver}:/opt/python-packages
EOF
    if [[ -d "${PROJECT_DIR}/enterprise_v${ver}" && "${INCLUDE_ENT}" == "yes" ]]; then
      echo "      - ${PROJECT_DIR}/enterprise_v${ver}:/mnt/enterprise" >> "$f"
    fi
    
    # CORRECCIÓN CRÍTICA: Configuración de puertos para modo local vs online
    if [[ "${DEPLOY_MODE}" == "local" ]]; then
      cat >> "$f" <<EOF
    ports:
      - "${odoo_port}:8069"
...
EOF
    ...
    environment:
      ...
      - USER=${PG_USER}
      - PASSWORD=${PG_PASS}
      # NUEVO: Variable de entorno para incluir el directorio de dependencias en Python path
...
networks:
  ${DOCKER_NET}:
    external: true
EOF
  ok "docker-compose.yml creado con volumen de dependencias persistentes"
}
...
deploy_containers() {
  ...
  # CORRECCIÓN: NO eliminar directorios PostgreSQL si ya existen datos
  ...
  # CORRECCIÓN: Iniciar solo PostgreSQL primero
  ...
  # CORRECCIÓN CRÍTICA: Crear usuario y base de datos POR DEFECTO
  for ver in "${ODOO_VERSIONS[@]}"; do
    local db_container="${PROJECT_NAME}_db${ver}"
    ...
    # CORRECCIÓN: Crear usuario si no existe
    ...
    # CORRECCIÓN: Crear base de datos por defecto
...
initialize_odoo_database() {
  ...
  # CORRECCIÓN: Usar la base de datos YA CREADA en deploy_containers
  for ver in "${ODOO_VERSIONS[@]}"; do
    local odoo_container="${PROJECT_NAME}_odoo${ver}"
    local odoo_port_var="ODOO_PORT_${ver}"
    local odoo_port="${!odoo_port_var:-8069}"
    ...
    # CORRECCIÓN: La base de datos YA debería existir (creada en deploy_containers)
    # Solo necesitamos inicializarla con Odoo
    ...
    if [[ "$db_exists" -gt 0 ]]; then
      ...
      # CORRECCIÓN: Inicializar Odoo en la base de datos existente
    ...
    # NUEVO: Informar sobre dependencias persistentes
    local py_packages_dir="${PROJECT_DIR}/python-packages_v${ver}"
    if [[ -d "$py_packages_dir" ]]; then
      local package_count=$(find "$py_packages_dir" -name "*.egg-info" -o -name "*.dist-info" 2>/dev/null | wc -l)
      if [[ $package_count -gt 0 ]]; then
        say "    📦 Dependencias persistentes: $package_count paquetes disponibles" "$C_GRN"
      else
        say "    💡 Usa 'Instalar dependencias persistentes' para añadir paquetes" "$C_BLU"
      fi
    fi
  done
  
  # Mostrar resumen de acceso
  print_success_box "🎉 CONFIGURACIÓN COMPLETA" "Base de datos: ${PROJECT_NAME}\nMaster Password: ${ADMIN_PASS}\nUsuario: ${ODOO_USER}\nContraseña: ${ODOO_PASS}\n\n📦 Sistema de dependencias persistentes activo"
...
# ====== FUNCIÓN PARA CONFIGURAR PROXY EN NPM ======
configure_npm_proxy() {
    local project_name="$1"
    local domain="$2"
    local email="$3"
    ...
    if ! check_npm_healthy; then
        ...
        say "   - Domain Names: ${domain}" "$C_BLU"
        say "   - Forward Hostname/IP: ${project_name}_odoo${ODOO_VERSIONS[0]}" "$C_BLU"
...
# ====== Limpieza de Proyecto (CORREGIDA - no falla si NPM no está instalado) ======
cleanup_project() {
...
# ====== Mantenimiento de Proyecto (CORREGIDA) ======
maintain_project() {
    ...
    while true; do
        ...
        case "$maint_opt" in
          ...
          6)
            ...
            ;;
          7)
            # NUEVA OPCIÓN: Instalar dependencias persistentes
            install_dependencies_persistent "$proj_name" "${ODOO_VERSIONS[0]}"
            read -p "Presiona Enter para continuar..."
            ;;
          8) say "Volviendo al menú principal..." "$C_BLU"; break;;
          *) warn "Opción no válida"; sleep 2;;
        esac
    done
}

# ====== Ver resumen de proyecto existente (CORREGIDA) ======
view_project_summary() {
  ...
  (
    ...
    # NUEVO: Información sobre dependencias persistentes
    for ver in "${ODOO_VERSIONS_ARR[@]}"; do
      local py_packages_dir="${PROJECT_DIR}/python-packages_v${ver}"
      if [[ -d "$py_packages_dir" ]]; then
        local package_count=$(find "$py_packages_dir" -name "*.egg-info" -o -name "*.dist-info" 2>/dev/null | wc -l)
        if [[ $package_count -gt 0 ]]; then
          say "📦 Dependencias persistentes v${ver}: $package_count paquetes" "$C_GRN"
        else
          say "📦 Dependencias persistentes v${ver}: Vacío (usa 'Instalar dependencias')" "$C_YLW"
        fi
      fi
    done
    
    print_info_box "🔧 COMANDOS ÚTILES" "cd ${PROJECT_DIR} && docker compose ps\ncd ${PROJECT_DIR} && docker compose logs -f ${PROJECT_NAME}_odoo${ODOO_VERSIONS_ARR[0]}\ncd ${PROJECT_DIR} && docker compose down\n\n📦 Para instalar dependencias:\nsudo ./$(basename "$0") --install-deps ${PROJECT_NAME} ${ODOO_VERSIONS_ARR[0]}"
...
install_dependencies_menu() {
    ...
    if [[ ${#existing_projects[@]} -eq 0 ]]; then
        warn "No hay proyectos disponibles"
        return
    fi
    
    echo -e "${C_CYN}Proyectos disponibles:${C_RESET}"
    local i=1
    for proj in "${existing_projects[@]}"; do
        echo -e "  ${C_GRN}$i)${C_RESET} ${C_BLU}$proj${C_RESET}"
        ((i++))
    ...
    read -p "Selecciona proyecto (número): " proj_num
    local selected_index=$((proj_num - 1))
    
    if [[ $selected_index -lt 0 || $selected_index -ge ${#existing_projects[@]} ]]; then
        warn "Selección inválida"
        return
    fi
    
    local selected_project="${existing_projects[$selected_index]}"
    
    # Preguntar versión de Odoo
    read -p "Versión de Odoo [19]: " odoo_version
    odoo_version="${odoo_version:-19}"
    
    # Llamar a la función de dependencias persistentes
    install_dependencies_persistent "$selected_project" "$odoo_version"
}
...
tools_menu() {
    while true; do
        ...
        read -p "Selecciona opción [1-8]: " tool_opt
...
# ====== Flujo de instalación principal MEJORADO ======
install_flow() {
  check_deps
  prompt_config
...
main_menu() {
    ...
    while true; do
        ...
        echo -e "${C_CYN}│ ${C_BLU}📊 INFORMACIÓN DEL SISTEMA${C_CYN}                               │${C_RESET}"
        echo -e "${C_CYN}├──────────────────────────────────────────────────────────┤${C_RESET}"
        echo -e "${C_CYN}│ ${C_YLW}📁 Ruta base:${C_RESET} ${BASE_DIR}${C_CYN}$(printf '%*s' "$((38-${#BASE_DIR}))" "")     │${C_RESET}"
        echo -e "${C_CYN}│ ${C_YLW}🌐 Panel NPM:${C_RESET} http://${NPM_PANEL_IP}:${NPM_PORT_WEB}${C_CYN}$(printf '%*s' "$((28-${#NPM_PANEL_IP}))" "")     │${C_RESET}"
        echo -e "${C_CYN}│ ${C_YLW}🔗 Red Docker:${C_RESET} ${COMMON_DOCKER_NETWORK}${C_CYN}$(printf '%*s' "$((36-${#COMMON_DOCKER_NETWORK}))" "")      │${C_RESET}"
        echo -e "${C_CYN}└──────────────────────────────────────────────────────────┘${C_RESET}"
        echo ""
...
if [[ "${1:-}" == "--install-deps" ]]; then
    if [[ -z "${2:-}" ]]; then
        echo "Uso: $0 --install-deps <nombre_proyecto> [odoo_version]"
        exit 1
    fi
    ...
    local quick_project="$2"
    local quick_version="${3:-19}"
    
    install_dependencies_persistent "$quick_project" "$quick_version"
    exit 0
fi

if [[ "${1:-}" == "--install-google-auth" ]]; then
    if [[ -z "${2:-}" ]]; then
        echo "Uso: $0 --install-google-auth <nombre_proyecto> [odoo_version]"
        exit 1
    fi
